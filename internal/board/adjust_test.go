...
func TestSpendShiftBudgetDeterministic(t *testing.T) {
	...
	if _, err := SpendShiftBudget(rand.New(rand.NewSource(3)), a, 4, 2, 0, 0, rules.Month{}); err != nil {
		t.Fatal(err)
	}
	if _, err := SpendShiftBudget(rand.New(rand.NewSource(3)), b, 4, 2, 0, 0, rules.Month{}); err != nil {
		t.Fatal(err)
	}
...
func TestSpendShiftBudgetZeroLeavesBoard(t *testing.T) {
	...
	before := prev.Clone()
	if _, err := SpendShiftBudget(rand.New(rand.NewSource(1)), prev, 0, 0, 0, 0, rules.Month{}); err != nil {
		t.Fatal(err)
	}
